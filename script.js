document.addEventListener('DOMContentLoaded', () => {
    ...
    /**
     * Download tailored resume as PDF file
     */
    function downloadTailoredResumeAsPdf() {
        ...
        doc.setProperties({
            title: 'JobGenie - Tailored Resume',
            subject: 'Tailored Resume',
            creator: 'JobGenie'
        });
        
        // Set font size and type
        doc.setFontSize(10); // Smaller font size to fit more content
        ...
        doc.text('JobGenie - Tailored Resume', pageWidth / 2, margin, { align: 'center' });
        ...
        for (let i = 0; i < sections.length; i++) {
            // Check if section is a header (all caps)
            const isHeader = sections[i].toUpperCase() === sections[i] && sections[i].length > 0;
            
            if (isHeader) {
                // Add some space before headers (except the first one)
                if (i > 0) y += lineHeight;
            ...
            const sectionLines = doc.splitTextToSize(sections[i], contentWidth);
            
            // Check if we need a new page
            if (y + (sectionLines.length * lineHeight) > pageHeight - margin) {
                doc.addPage();
                y = margin; // Reset y position for new page
            }
            
            // Add section lines to PDF
            for (let j = 0; j < sectionLines.length; j++) {
                doc.text(sectionLines[j], margin, y);
                y += lineHeight;
            }
